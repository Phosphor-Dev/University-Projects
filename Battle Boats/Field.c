...
void FieldInit(Field *own_field, Field * opp_field){
    ...
    //CREATES THEIR GRID PATTERNS
    own_field->grid[FIELD_ROWS][FIELD_COLS];
    opp_field->grid[FIELD_ROWS][FIELD_COLS];
    //FILLS ownField WITH FIELD_SQUARE_EMPTY
    for(a=0;a<FIELD_ROWS;a++){
        for(b=0;b<FIELD_COLS;b++){
            own_field->grid[a][b] = FIELD_SQUARE_EMPTY;
        }
    }
    //FILLS enemyField WITH FIELD_SQUARE_UNKNOWN
    for(a=0;a<FIELD_ROWS;a++){
        for(b=0;b<FIELD_COLS;b++){
            opp_field->grid[a][b] = FIELD_SQUARE_UNKNOWN;
        }
    }
    //FILL ENENY BOATLIVES PARAMETER 
...
SquareStatus FieldGetSquareStatus(const Field *f, uint8_t row, uint8_t col){
    //CHECKS IF THE COORDINATES SENT ARE VALID, IF NOT RETURN FIELD_SQUARE_INVALID
    //IF COORDIANTES ARE VALID, RETURN THE VALUE AT COORDINATES
    if ((row > 6) || (col > 10)){
...
SquareStatus FieldSetSquareStatus(Field *f, uint8_t row, uint8_t col, SquareStatus p){
    //CREATES oldValue TO SAVE PREVIOUS DATA, THEN OVERWRITES THE DATA AT THAT COORDINATE
    int oldValue = f->grid[row][col];
    f->grid[row][col] = p;
    return oldValue;
}
...
uint8_t FieldAddBoat(Field *own_field, uint8_t row, uint8_t col, BoatDirection dir, BoatType boat_type){
    int a,b,c;
    if (boat_type==FIELD_BOAT_TYPE_SMALL){b = 3; own_field->smallBoatLives = 3; c = FIELD_SQUARE_SMALL_BOAT;}
    else if (boat_type==FIELD_BOAT_TYPE_MEDIUM){b = 4; own_field->mediumBoatLives = 4; c = FIELD_SQUARE_MEDIUM_BOAT;}
    else if (boat_type==FIELD_BOAT_TYPE_LARGE){b = 5; own_field->largeBoatLives = 5; c = FIELD_SQUARE_LARGE_BOAT;}
    else {b = 6; own_field->hugeBoatLives = 6; c = FIELD_SQUARE_HUGE_BOAT;}
    //FIRST CHECKS IF STARTING COORDINATE IS VALID OR NOT EMPTY
    //THEN CHECKS SOUTH DIRECTION, IF SPACE IS VALID IT WILL PLACE THE SHIP
    //THEN CHECKS EAST DRIECTION, IF SPACE IS VALID IT WILL PLACE THE SHIP
    if ((FieldGetSquareStatus(own_field, row, col) == FIELD_SQUARE_INVALID) || ((FieldGetSquareStatus(own_field, row, col) != FIELD_SQUARE_EMPTY) && (FieldGetSquareStatus(own_field, row, col) != FIELD_SQUARE_UNKNOWN))){
        return STANDARD_ERROR;
    } else if ((dir == FIELD_DIR_SOUTH) && (row+b <= 6)){
        for(a=row;a<(row+b);a++)
            if((FieldGetSquareStatus(own_field, a, col) != FIELD_SQUARE_EMPTY) && (FieldGetSquareStatus(own_field, a, col) != FIELD_SQUARE_UNKNOWN))
                return STANDARD_ERROR;   
        for(a=row;a<(row+b);a++)
            FieldSetSquareStatus(own_field, a, col, c);     
        return SUCCESS;
    } else if ((dir == FIELD_DIR_EAST) && (col+b <= 10)){
        for(a=col;a<(col+b);a++)
            if((FieldGetSquareStatus(own_field, row, a) != FIELD_SQUARE_EMPTY) && (FieldGetSquareStatus(own_field, row, a) != FIELD_SQUARE_UNKNOWN))
                return STANDARD_ERROR;   
        for(a=col;a<(col+b);a++)
            FieldSetSquareStatus(own_field, row, a, c);
        return SUCCESS;
    } else {
        return STANDARD_ERROR;
    }
...
SquareStatus FieldRegisterEnemyAttack(Field *own_field, GuessData *opp_guess){
    int previousData = own_field->grid[opp_guess->row][opp_guess->col];
    if (own_field->grid[opp_guess->row][opp_guess->col] != 0){
        opp_guess->result = RESULT_HIT;
        if(own_field->grid[opp_guess->row][opp_guess->col] == FIELD_SQUARE_HUGE_BOAT){
            own_field->hugeBoatLives -= 1;
            if (own_field->hugeBoatLives == 0){opp_guess->result = RESULT_HUGE_BOAT_SUNK;}  
            ...
            if (own_field->largeBoatLives == 0){opp_guess->result = RESULT_LARGE_BOAT_SUNK;}   
            //own_field->grid[opp_guess->row][opp_guess->col] = FIELD_SQUARE_HIT;
        }
        else if(own_field->grid[opp_guess->row][opp_guess->col] == FIELD_SQUARE_MEDIUM_BOAT){
            own_field->mediumBoatLives -= 1;
            if (own_field->mediumBoatLives == 0){opp_guess->result = RESULT_MEDIUM_BOAT_SUNK;}   
            //own_field->grid[opp_guess->row][opp_guess->col] = FIELD_SQUARE_HIT;
        }
        else if(own_field->grid[opp_guess->row][opp_guess->col] == FIELD_SQUARE_SMALL_BOAT){
            own_field->smallBoatLives -= 1;
            if (own_field->smallBoatLives == 0){opp_guess->result = RESULT_SMALL_BOAT_SUNK;}   
            //own_field->grid[opp_guess->row][opp_guess->col] = FIELD_SQUARE_HIT;
        }
    } else {
...
SquareStatus FieldUpdateKnowledge(Field *opp_field, const GuessData *own_guess){
    //GuessData oldGuess; oldGuess.row = own_guess->row; oldGuess.col = own_guess->col;
    ...
    else if (own_guess->result == RESULT_SMALL_BOAT_SUNK){opp_field->smallBoatLives = 0; opp_field->grid[own_guess->row][own_guess->col] = FIELD_SQUARE_HIT; }
    else if (own_guess->result == RESULT_MEDIUM_BOAT_SUNK){opp_field->mediumBoatLives = 0; opp_field->grid[own_guess->row][own_guess->col] = FIELD_SQUARE_HIT; }
    else if (own_guess->result == RESULT_LARGE_BOAT_SUNK){opp_field->largeBoatLives = 0; opp_field->grid[own_guess->row][own_guess->col] = FIELD_SQUARE_HIT; }    
    else if (own_guess->result == RESULT_HUGE_BOAT_SUNK){opp_field->hugeBoatLives = 0; opp_field->grid[own_guess->row][own_guess->col] = FIELD_SQUARE_HIT; }
...
uint8_t FieldGetBoatStates(const Field *f){
    int output = 0;
    if (f->smallBoatLives != 0){ output += 1;}
    if (f->mediumBoatLives != 0){ output += 2;}
    if (f->largeBoatLives != 0){ output += 4;}
    if (f->hugeBoatLives != 0){ output += 8;}
    return output;
}
...
uint8_t FieldAIPlaceAllBoats(Field *own_field){
    ...
    while(runtime != 4){
        ...
        int shipPlacedSucuess = 0;
        if(direction == 1){ direction = FIELD_DIR_SOUTH;}
        else {direction = FIELD_DIR_EAST;}
    
        while(shipPlacedSucuess != 1){
            row = rand() % 6;
            col = rand() % 10;
                if (FieldAddBoat(own_field, row, col, direction, runtime) != STANDARD_ERROR){
                    shipPlacedSucuess = 1;
                }
            }
        runtime += 1;
...
GuessData FieldAIDecideGuess(const Field *opp_field){
    GuessData Guess;
    static int availableSquares[6][10];
    int row,col;
    int squareChecked = 0;
    
    while(squareChecked != 1){
        row = rand() % 6;
        col = rand() % 10;
        if (availableSquares[row][col] == 0){
            availableSquares[row][col] = 1;
            Guess.row = row;
            Guess.col = col;
            squareChecked = 1;
        } 
