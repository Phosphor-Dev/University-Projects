...
LIST_ELEMENT List::moveNext()
{
    ...
    LIST_ELEMENT element = afterCursor->data;
...
LIST_ELEMENT List::movePrev()
{
    ...
    LIST_ELEMENT element = beforeCursor->data;
...
bool List::equals(const List &R)
{
    ...
    LIST_ELEMENT length = R.num_elements;
    if (this->num_elements != R.num_elements) return false;
    else
    {
        while (length != 0)
        {
            LIST_ELEMENT temp = N->data;
            LIST_ELEMENT temp2 = M->data;
            N = N->next;
            M = M->next;
